...
uninstall='sudo /usr/local/node/bin/npm -g uninstall'
if [ 1 = ${DEBUG} ];    then
...
npmdir="${HOME}/.npm" #read -p "Choose your install directory. Default (${defaultnpmdir}) : " npmdir
...
yn="n" #read -p "Do you wish to update your .bashrc/.zshrc file(s) with the paths and manpaths? [yn] " yn
case $yn in
    [Yy]* ) fix_env;;
    [Nn]* ) echo_env;;
